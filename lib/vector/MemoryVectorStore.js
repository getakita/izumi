...
export class MemoryVectorStore {
    ...
    ddlItems = [];
    documentationItems = [];
    async addQuestionSQL(question, sql, embedding) {
    ...
    async addDDL(ddl, embedding, tableName) {
    ...
    async addDocumentation(documentation, embedding, title) {
    ...
    async getSimilarQuestionSQL(queryEmbedding, limit = 5) {
    ...
    async getRelatedDDL(queryEmbedding, limit = 10) {
    ...
    async getRelatedDocumentation(queryEmbedding, limit = 5) {
    ...
    async removeTrainingData(id) {
        ...
        return (initialLengths.qs !== finalLengths.qs ||
    ...
    async findDDLByTableName(tableName) {
        return this.ddlItems.filter(item => item.table_name?.toLowerCase() === tableName.toLowerCase() ||
            item.ddl.toLowerCase().includes(tableName.toLowerCase()));
    }
    async searchQuestionSQL(query) {
        const queryLower = query.toLowerCase();
        return this.questionSQLPairs.filter(item => item.question.toLowerCase().includes(queryLower) ||
