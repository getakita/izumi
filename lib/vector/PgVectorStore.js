...
export class PgVectorStore {
    ...
    async storeQuestionSQL(item) {
        ...
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO ${this.schema}.question_sql_pairs (question, sql, embedding) 
         VALUES ($1, $2, $3::vector)
         ON CONFLICT DO NOTHING`, [item.question, item.sql, `[${item.embedding.join(',')}]`]);
        }
        finally {
            client.release();
        }
    ...
    async storeDDL(item) {
        ...
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO ${this.schema}.ddl_items (ddl, table_name, embedding) 
         VALUES ($1, $2, $3::vector)
         ON CONFLICT DO NOTHING`, [item.ddl, item.table_name || null, `[${item.embedding.join(',')}]`]);
        }
        finally {
            client.release();
        }
     ...
     */
    async storeDocumentation(item) {
        if (!item.embedding) {
            throw new Error('Embedding is required for storage');
        }
        const client = await this.pool.connect();
        try {
            await client.query(`INSERT INTO ${this.schema}.documentation_items (content, title, embedding) 
         VALUES ($1, $2, $3::vector)
         ON CONFLICT DO NOTHING`, [item.documentation, item.title || null, `[${item.embedding.join(',')}]`]);
        }
        finally {
            client.release();
        }
     ...
     */
    async findSimilarQuestions(embedding, limit = 5) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT question, sql, (1 - (embedding <=> $1::vector)) as similarity
     ...
     */
    async findSimilarDDL(embedding, limit = 5) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT ddl, table_name, (1 - (embedding <=> $1::vector)) as similarity
     ...
     */
    async findSimilarDocumentation(embedding, limit = 5) {
        const client = await this.pool.connect();
        try {
            const result = await client.query(`SELECT content, title, (1 - (embedding <=> $1::vector)) as similarity
